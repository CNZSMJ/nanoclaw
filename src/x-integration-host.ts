/**
 * X Integration Host-Side IPC Handler
 *
 * Handles x_* IPC messages sent by container agents via the task IPC file mechanism.
 * Spawns local scripts that use Playwright + the user's real Chrome to perform X actions.
 * Writes results back to the IPC results directory for the container to poll.
 */
...
// Run a skill script as subprocess using tsx
...
// Write result to IPC results directory for container to poll
...
/**
 * Handle X integration IPC messages dispatched from processTaskIpc.
 *
 * @returns true if message was handled (x_* type), false otherwise
 */
export async function handleXIpc(
  ...
  if (!requestId) {
    logger.warn({ type }, 'X integration request missing requestId');
    return true;
  }
  ...
  switch (type) {
    ...
    case 'x_view_profile':
      ...
      result = await runScript('view_profile', {
        username: data.username,
        maxTweets: data.maxTweets || 5,
      });
      break;
